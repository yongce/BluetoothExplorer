 ...
 *
 * Copied from the project https://github.com/androidthings/sample-bluetooth-le-gattserver.
 */
object TimeServiceProfile {
    ...
    fun getExactTime(timestamp: Long, adjustReason: Byte): ByteArray {
        ...
        // Day [1, 31]
        field[3] = time.get(Calendar.DAY_OF_MONTH).toByte()
        // Hours [0, 23]
        ...
        // Day of Week (1-7)
        field[7] =
            getDayOfWeekCode(time.get(Calendar.DAY_OF_WEEK))
        // Fractions256
    ...
    fun getLocalTimeInfo(timestamp: Long): ByteArray {
        ...
        // Time zone (15 minute intervals)
        val zoneOffset = time.get(Calendar.ZONE_OFFSET) / FIFTEEN_MINUTE_MILLIS
        field[0] = zoneOffset.toByte()

        // DST Offset (30 minute intervals)
        val dstOffset = time.get(Calendar.DST_OFFSET) / HALF_HOUR_MILLIS
